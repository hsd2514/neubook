...
def test_booking_lifecycle_completed_flow(client, db_session):
    ...
    slot_start = datetime.now(timezone.utc).replace(microsecond=0) + timedelta(days=1)
    booking_payload = {
...
def test_booking_publication_and_capacity_invariants(client, db_session):
    ...
    slot_start = datetime.now(timezone.utc).replace(microsecond=0) + timedelta(days=2)
    blocked_payload = {
