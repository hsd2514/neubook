"""backfill seat map foundation for legacy 009 state
...
def upgrade() -> None:
    ...
    seat_block_indexes = {idx["name"] for idx in inspector.get_indexes("seat_blocks")} if "seat_blocks" in set(inspector.get_table_names()) else set()
    if "ix_seat_blocks_appointment_type_id" not in seat_block_indexes:
        op.create_index("ix_seat_blocks_appointment_type_id", "seat_blocks", ["appointment_type_id"])
    if "ix_seat_blocks_resource_id" not in seat_block_indexes:
    ...
    seat_indexes = {idx["name"] for idx in inspector.get_indexes("seats")} if "seats" in set(inspector.get_table_names()) else set()
    if "ix_seats_appointment_type_id" not in seat_indexes:
        op.create_index("ix_seats_appointment_type_id", "seats", ["appointment_type_id"])
    if "ix_seats_block_id" not in seat_indexes:
        op.create_index("ix_seats_block_id", "seats", ["block_id"])
    if "ix_seats_resource_id" not in seat_indexes:
    ...
    booking_seat_indexes = {idx["name"] for idx in inspector.get_indexes("booking_seats")} if "booking_seats" in set(inspector.get_table_names()) else set()
    if "ix_booking_seats_booking_id" not in booking_seat_indexes:
        op.create_index("ix_booking_seats_booking_id", "booking_seats", ["booking_id"])
    if "ix_booking_seats_seat_id" not in booking_seat_indexes:
